import type { HeroContent } from "@/lib/content-defaults";
import { defaultHero } from "@/lib/content-defaults";

export default function Hero({ content = defaultHero }: { content?: HeroContent }) {
  const waLink = `https://wa.me/${content.whatsapp_number}`;

  return (
    <section id="inicio" className="bg-bg py-20 md:py-28">
      <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
        {/* Left: Content */}
        <div>
          {/* Badge pills */}
          <div className="flex flex-wrap gap-2 mb-6">
            {content.badges.map((badge) => (
              <span
                key={badge}
                className="bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-medium"
              >
                {badge}
              </span>
            ))}
          </div>

          <h1 className="font-serif text-4xl md:text-5xl font-bold text-dark leading-tight mb-4">
            {content.headline_part1}{" "}
            <span className="text-primary">{content.headline_highlight}</span>{" "}
            {content.headline_part2}
          </h1>

          <p className="text-base text-muted mb-6 leading-relaxed">
            {content.subheadline}
          </p>

          <p className="text-dark/70 mb-8 leading-relaxed">{content.body}</p>

          <a
            href={waLink}
            className="inline-flex items-center gap-2 bg-accent text-white px-6 py-3 rounded-full text-base font-semibold hover:bg-accent-dark transition-colors shadow-md"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.890-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            {content.cta_text}
          </a>
        </div>

        {/* Right: Profile / Photo */}
        <div className="flex items-center justify-center">
          <div className="relative w-80 h-80">
            {content.image ? (
              /* CMS-uploaded photo */
              <>
                <div className="absolute inset-0 bg-primary/10 rounded-full" />
                <div className="absolute inset-4 overflow-hidden rounded-full">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={content.image}
                    alt={content.profile_name}
                    className="w-full h-full object-cover"
                  />
                </div>
              </>
            ) : (
              /* Decorative placeholder */
              <>
                <div className="absolute inset-0 bg-primary/10 rounded-full" />
                <div className="absolute inset-6 bg-primary/10 rounded-full" />
                <div className="absolute inset-12 bg-primary/15 rounded-full" />
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-8">
                  <div className="w-16 h-16 bg-primary rounded-full flex items-center justify-center text-white text-3xl mb-4">
                    🧠
                  </div>
                  <p className="font-serif text-xl font-bold text-primary leading-tight">
                    {content.profile_name}
                  </p>
                  <p className="text-xs text-muted mt-2 leading-relaxed">
                    {content.profile_subtitle}
                  </p>
                  {content.profile_tags.length > 0 && (
                    <div className="mt-4 flex gap-2 flex-wrap justify-center">
                      {content.profile_tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-xs bg-accent/10 text-accent px-2 py-0.5 rounded-full"
                        >
                          {tag}
                        </span>
                      ))}
                    </div>
                  )}
                </div>
              </>
            )}
          </div>
        </div>
      </div>
    </section>
  );
}
